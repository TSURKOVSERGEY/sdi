...
void WriteNandPage(adpcm_message_struct *padpcm_msg)
{
  ...
  {
    if(CheckBlockGetPageAdr()) // œ–Œ¬≈– ¿ ÷≈ÀŒ—“ÕŒ—“» ¡ÀŒ ¿ œŒƒ «¿√ŒÀŒ¬Œ 
    {
      ...
      if(CheckBlockGetPageAdr()) // œ–Œ¬≈– ¿ ÷≈ÀŒ—“ÕŒ—“» ¡ÀŒ ¿ œŒƒ ƒ¿ÕÕ€≈ (Õ”À≈¬¿ﬂ —“–¿Õ»÷¿)
      {
  ...
  {
    if(CheckBlockGetPageAdr()) // œ–Œ¬≈– ¿ ÷≈ÀŒ—“ÕŒ—“» ¡ÀŒ ¿ œŒƒ ƒ¿ÕÕ€≈ (œŒ—À≈ƒÕﬂﬂ —“–¿Õ»÷¿)
    {
      ...
      if(CheckBlockGetPageAdr()) // œ–Œ¬≈– ¿ ÷≈ÀŒ—“ÕŒ—“» ¡ÀŒ ¿ œŒƒ «¿√ŒÀŒ¬Œ  —À≈ƒ”ﬁŸ≈√Œ —”œ≈– ¡ÀŒ ¿
	  {
  ...
  {
    if(CheckBlockGetPageAdr())  // œ–Œ¬≈– ¿ ÷≈ÀŒ—“ÕŒ—“» ¡ÀŒ ¿ œŒƒ ƒ¿ÕÕ€≈ (“≈ ”Ÿ¿ﬂ —“–¿Õ»÷¿)
    {
      if(nand_write_page((uint16_t*)&padpcm_msg,alarm_data.PageAddress,fp[1]) == 1)
	  {
		  alarm_data.PageRealWrite++; 
...
void rewrite_block(int mode_rewrite,uint16_t *page_bufer_old)
{
	...
	if(CheckBlockGetPageAdr()) // œ–Œ¬≈– ¿ ÷≈ÀŒ—“ÕŒ—“» ¡ÀŒ ¿ œŒƒ ƒ¿ÕÕ€≈ (œ≈–≈«¿œ»—€¬¿≈Ã€ ¡ÀŒ )
    {
		for(uint32_t i = 0; i < byte_real_write; i++)
		{
		   ...
		   else rewrite_block(REWRITE_BLOCK_NEXT,page_bufer_old);
		}


	    if(nand_write_page(page_bufer_old,alarm_data.PageAddress,fp[1]) == 1)
		{
			 alarm_data.PageRealWrite++; 
 		     alarm_data.PageAddress++;
		}
