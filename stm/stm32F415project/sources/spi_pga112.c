...
void SPI_PGA112Config(void)
{
  ...
  for(int i = 0; i < MAX_CHANNEL; i++) 
  {
    ...
    GPIO_WriteBit(GPIO_PORT[i],GPIO_PIN[i],Bit_SET);
...
void SPI_PGA112_CS_GPIO_Config(CS_TypeDef CS)
{
  ...
  RCC_AHB1PeriphClockCmd(GPIO_CLK[CS], ENABLE);
...
void PGA112_WriteData(int CS, unsigned short gain, unsigned short channel)
{
  ...
  GPIO_WriteBit(GPIO_PORT[CS],GPIO_PIN[CS],Bit_RESET);
  ...
  //PGA112_Clr_CS((CS_TypeDef)CS);
    GPIO_WriteBit(GPIO_PORT[CS],GPIO_PIN[CS],Bit_SET);
